            ...
            $.each(inputs, $.proxy(function(i, input) {
			  ...
			  else if(input.type == 'radio')
			  {
				...
				if(input.checked===input.value){
					useFlag = 'checked="checked"';
				} else {
					useFlag = '';
				}
				iLabel = input.label ? '<label class="'+this.options.name+'-label" style="display:inline-block;">'+input.label : '';
				...
				this.esqueleto.inputs.append($(iLabel+'<input type="'+input.type+'" name="useFlag" autocomplete="off" value="'+input.value+'" style="display: inline-block;width: 50px;"' + useFlag +iRequired+' />'+fLabel));
			  }
              else
              {
