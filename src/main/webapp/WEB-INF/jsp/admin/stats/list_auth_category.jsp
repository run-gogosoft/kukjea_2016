...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>인증기업별 매출 현황 <small></small></h1>
		<ol class="breadcrumb">
			<li><a href="/admin/index"><i class="fa fa-home"></i>Home</a></li>
			<li>통계</li>
			<li class="active">인증기업별 매출 현황</li>
		</ol>
	</section>
				...
				<div class="box">
					...
					<div class="box-body">
						<table class="table table-bordered table-striped">
							<colgroup>
								<col style="width:5%;"/>
								<c:forEach items="${vo.authCategoryList}">
								<col style="width:9%;"/>
								</c:forEach>
								<col style="width:5%;"/>
							</colgroup>
							<thead>
								<tr>
									<th>결제 일자</th>
									<c:forEach var="item" items="${vo.authCategoryList}">
									<th>${item.name}</th>	
									</c:forEach>
									<th>합계</th>
								</tr>
							...
							<c:forEach var="item" items="${list}">
								...
								<tr>
									...
									<c:forEach items="${vo.authCategoryList}" varStatus="status">
							...
							<c:if test="${ fn:length(list)==0 }">
								<tr><td class="text-center" colspan="${fn:length(vo.authCategoryList)+2}">조회된 데이터가 없습니다.</td></tr>
							</c:if>
							<c:set var="sumRow" value="0"/>
								<tr>
...
<script type="text/javascript">
	...
	var drawChart = function() {
		new Morris.Bar({
			...
			resize: true,
			data: [
			<c:forEach var="item" items="${vo.authCategoryList}" varStatus="status">
				<c:set var="propName" value="sumPrice${status.index+1}"/>
