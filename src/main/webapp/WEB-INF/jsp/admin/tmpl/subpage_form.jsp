...
<div class="content-wrapper">
	...
	<section class="content-header">
		<h1>서브페이지 관리 <small></small></h1>
		<ol class="breadcrumb">
			<li><a href="/admin/index"><i class="fa fa-home"></i>Home</a></li>
			<li>시스템 관리</li>
			<li>템플릿 관리</li>
			<li class="active">서브페이지 관리</li>
		</ol>
	</section>
	<!-- 콘텐츠 -->
			...
			<div class="col-md-12">
				<div class="box">
					...
					<div class="box-header with-border">
						...
						<select alt="회원구분 리스트" id="memberTypeList" class="fix-height" style="margin-left:5px" onchange="memberTypeSelect()">
							<option value="0">-구분 선택-</option>
							<c:forEach var="item" items="${memberTypeList}">
								<option value="${item.value}" ${memberTypeCode eq item.value ? "selected" : ""}>${item.name}</option>
							</c:forEach>
						</select>
						...
						<select alt="대분류 카테고리" id="lv1List" class="fix-height" style="margin-left:5px" onchange="categorySelect()"><option value="0">-카테고리 선택-</option></select>
					</div>
					<!-- 내용 -->
					<div class="box-body">
						<div class="col-md-3">
							<div class="non-deco" style="width:250px">
								...
								<div class="clearfix t-margin">
									<div style="float:left;width:34%;height:69px">
										<div class="round t-padding" style="background:#4C4C4C;color:red;height:69px;line-height:40px;">menu</div>
									</div>
									<!-- main hero banner -->
									<div id="outer" class="round hand div-over t-padding" style="float:right;width:65%;" onclick="goStyle('1','3','0', this)">
										<div id="middle" style="width:145px;">
											<div id="inner styleBanner1">A itemlist</div>
										</div>
									</div>
								</div>
								<!-- B itemlist -->
								<div class="round hand div-over clearfix t-margin t-padding styleBanner2" onclick="goStyle('2','4','1', this)">B itemlist</div>
								<!-- C itemlist -->
								<!-- <div class="round hand div-over clearfix t-margin t-padding styleBanner3" onclick="goStyle('3','15','2', this)">C itemlist</div> -->
						...
						<div class="col-md-9" id="htmlDiv">
							<div class="widget">
								<form id="htmlForm" method="post" onsubmit="return submitProc(this);" target="zeroframe">
									...
									<input type="hidden" name="cateSeq" id="cateSeq"/>
						...
						<div class="col-md-9" id="styleDiv" style="display:none;">
							<div class="widget">
								<div class="widget-header" style="line-height:0;">
									...
									<form action="/admin/system/sub/title/edit/proc" onsubmit="return submitProc(this);" method="post" class="pull-right" target="zeroframe">
										<div class="input-append" style="margin-top:3px;">
											...
											<input type="hidden" name="cateSeq" id="titleCateSeq"/>
											<input type="hidden" name="memberTypeCode" id="titleMemberTypeCode"/>
										</div>
									</form>
								</div>
								<div class="widget-content">
									<form action="/admin/system/sub/orderno/edit/proc" onsubmit="return submitProc(this);" method="post" target="zeroframe">
										<input type="hidden" name="styleCode" id="modOrderNoStyleCode"/>
										<input type="hidden" name="cateSeq" id="modOrderNoCateSeq"/>
						...
						<div class="widget widget-form stacked">
							<div class="widget-header">
								...
								<div class="pull-right">
									...
									<button type="button" onclick="batchShow({'method':'등록','action':'reg', 'statusCode':'Y', 'cateSeq':0})" class="btn btn-small btn-success">상품등록</button>
								</div>
							</div>
							<div class="widget-content">
								<div class="row-fluid">
								...
								<div class="row-fluid">
									<div class="control-group col-md-6">
										...
										<div class="controls">
											<input type="text" name="name" id="searchItemName" class="col-md-12 fix-height" onkeypress="event.keyCode==13 && goPage(0)" />
										</div>
									</div>
									<div class="control-group col-md-6">
										...
										<div class="controls">
											<input type="text" name="searchItemSeq" id="searchItemSeq" onblur="numberCheck(this);" class="col-md-12 fix-height"  onkeypress="event.keyCode==13 && goPage(0)" />
										</div>
									</div>
								</div>
							</div>
...
<div id="deleteModal" class="modal">
	<div class="modal-dialog">
			<div class="modal-content">
				<form action="/admin/system/sub/item/delete/proc" method="post" target="zeroframe">
...
<script id="batchTemplate" type="text/html">
	<form action="/admin/system/sub/item/reg/proc" onsubmit="return batchSubmitProc(this)" method="post" target="zeroframe">
		<input type="hidden" name="displaySeq" value="<%="${displaySeq}"%>"/>
		<input type="hidden" name="memberTypeCode" value="<%="${memberTypeCode}"%>"/>
		<input type="hidden" name="styleCode" value="<%="${styleCode}"%>"/>
		<input type="hidden" name="cateSeq" value="<%="${cateSeq}"%>"/>
		<div class="modal-body">
...
<form id="regStyleForm" action="/admin/system/sub/style/reg/proc" method="post" target="zeroframe">
	<input type="hidden" name="cateSeq" id="regStyleCateSeq"/>
	...
	$.ajax({
		...
		success:function(data) {
			...
			if(list.length === 0){
				...
				$('#lv1List').html('<option value="0">--카테고리 선택--</option>');
				$('#cateLv1Seq').html('<option value="0">--카테고리 선택--</option>');
			} else {
				//최초 몰선택시 좌측상단, 상품검색 대분류 카테고리를 가져온다.
				$('#lv1List').html($("#categoryTemplate").tmpl(valueList));
...
var memberTypeSelect = function(){
	var memberTypeCode = $('#memberTypeList option:selected').val();
	if(parseInt($('#lv1List option:selected').val(),10) !== 0){
		$('#lv1List option:first').prop('selected',true);
		initSubPage();
	}
...
var categorySelect = function(){
	var memberTypeCode = $('#memberTypeList option:selected').val();
	if(memberTypeCode === ''){
		alert('회원구분이 선택되지 않았습니다.');
		$('#lv1List option:first').prop('selected',true);
	} else {
		//카테고리가 변경되었다면 초기화면으로 돌아간다.
		initSubPage();
	}
};

var renderSubPage = function(location, title, cateSeq, memberTypeCode, styleCode){
...
var callbackProc = function(msg) {
	...
	if(parseInt(callBackMsg.length,10) === 2){ //length == 2 이라면 상품추가
		renderSubPage('', '', '', callBackMsg[0], callBackMsg[1]);
	}
	else if(parseInt(callBackMsg.length,10) === 4){ //length == 4 이라면 배너 등록. 수정
		//0:location, 1:title, 2:cateSeq, 3:mallSeq
		renderSubPage(callBackMsg[0], callBackMsg[1], callBackMsg[2] , callBackMsg[3], '');
	} else if(parseInt(callBackMsg.length,10) === 3){ //length == 3 이라면 스타일(상품군) 등록
		//0:cateSeq, 1:mallSeq, 2:styleCode
		renderSubPage('', '', callBackMsg[0] , callBackMsg[1], callBackMsg[2]);
	} else if(parseInt(callBackMsg.length,10) === 8){ //length == 8 상품군에 대한 수정(제목, 정렬순서, 상품삭제)
		//0:cateSeq, 1:mallSeq, 2:styleCode, 3:lv1, 4:lv2, 5:lv3, 6:itemSeq, 7:itemName
...
var submitProc = function (obj) {
	...
	var cateSeq = parseInt($('#lv1List option:selected').val(),10);
	if (cateSeq === 0) {
...
var getDisplayVo = function (location, title) {
	...
	var cateSeq = parseInt($('#lv1List option:selected').val(),10) || 0;
	if (cateSeq === 0) {
		alert('카테고리가 선택되지 않았습니다.');
		return;
	}
	...
	$.ajax({
		type: "GET",
		url: "/admin/system/tmpl/sub/ajax",
		dataType: "text",
		data: {location: location, title: title, cateSeq: cateSeq, memberTypeCode:memberTypeCode},
		success: function (data) {
			...
			if (vo.title === "" && vo.content === "") {
				...
				$('#htmlForm').attr('action', '/admin/system/sub/reg/proc');
			} else {
				...
				$('#htmlForm').attr('action', '/admin/system/sub/edit/proc');
...
var goStyle = function (styleCode,limitCnt,orderNo, obj) {
	...
	$('#searchSucSeq').val(cateSeq);
	$('#searchLv1Seq').val(cateSeq);
	$("#searchMemberTypeCode").val(memberTypeCode);
	$("#searchStyleCode").val(styleCode);
	//상품 추가시 필요한 요소들
	$('#tempCateSeq').val(cateSeq);
	$("#tempStyleCode").val(styleCode);
	$("#tempMemberTypeCode").val(memberTypeCode);

	$("#StyleTitle").html("<span style='display:inline-block;margin-top:10px;color:blue'>" + (obj === '.styleBanner1' ? 'A itemlist' : $(obj).text()) + "</span>");
	...
	$.ajax({
		...
		dataType: "text",
		data: {styleCode: styleCode, cateSeq:cateSeq, memberTypeCode:memberTypeCode},
		success: function (data) {
...
//					$("#displaySeq").val($.parseJSON(data).seq);//sm_display_item의 seq를 가지고 있음
					...
					$.ajax({
						...
						dataType: "text",
						data: {styleCode: styleCode, cateSeq:cateSeq, displaySeq:displaySeq,memberTypeCode:memberTypeCode},
						success: function (data) {
							var list = $.parseJSON(data);
