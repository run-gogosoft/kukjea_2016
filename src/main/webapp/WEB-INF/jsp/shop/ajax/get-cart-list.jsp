...
<c:forEach var="item" items="${list}" varStatus="status" begin="0" step="1">
	<c:if test="${status.index ne 0}">,</c:if>
	{
		...
		, "count":"${item.count}"
		, "sellPrice":"${item.sellPrice+item.optionPrice}"
		, "sellPriceText":"<fmt:formatNumber value="${item.sellPrice+item.optionPrice}" pattern="#,###" />"
		, "totalPriceText":"<fmt:formatNumber value="${(item.sellPrice+item.optionPrice)*item.count}" pattern="#,###" />"
