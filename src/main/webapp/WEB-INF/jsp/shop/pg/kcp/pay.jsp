...
<%@ page import="com.smpro.util.pg.PgUtil" %>
...
%><%
	...
	/* 복합과세 설정 */
	int commFreeMny = (Integer)request.getAttribute("taxFreeAmt"); //면세금액
	int commTaxMny = 0;  //과세금액
	int commTaxSupplyMny  = 0; //과세 공급가액
	int commTaxVatMny  = 0; //부가세
	if( commFreeMny > 0) {
		if(vo.getPayPrice() < commFreeMny) {
			commFreeMny = vo.getPayPrice(); //면세금액이 결제금액보다 클경우 최대 금액을 결제금액으로 설정한다.
		}
		commTaxMny = vo.getPayPrice() - commFreeMny;
		if(commTaxMny >0) {
			commTaxSupplyMny = (int)(Math.ceil((commTaxMny) / 1.1));
			commTaxVatMny = commTaxMny - commTaxSupplyMny;
		}
	}
%>
...
%>
    <script type="text/javascript">
        ...
        {
            openwin = window.open( "/front-assets/kcp/chk_plugin.html", "chk_plugin", "width=420, height=100, top=300, left=300" );
        }
        */
        ...
        function  jsf__pay( form )
        {
            ...
            {
                //openwin = window.open( "/front-assets/kcp/proc_win.html", "proc_win", "width=449, height=209, top=300, left=300" );
                RetVal = true ;
            }
...
<form name="order_info" method="post" action="/shop/${mallId}/order/result" target="zeroframe">
<%
					/* ============================================================================== */
					/* =   1. 주문 정보 입력                                                        = */
					/* = -------------------------------------------------------------------------- = */
					...
					/* = -------------------------------------------------------------------------- = */
                    /* [결제요청] 이 페이지는 결제를 요청하는 샘플(예시) 페이지입니다.
                             고객이 결제요청 페이지에 접근하게 되면 웹 브라우저에서 결제를 위한 Payplus Plug-in이
                             고객의 PC에 설치되어 있는지 확인합니다.
                             고객의 PC에 Payplus Plug-in이 설치되지 않은 경우 브라우저 상단의 노란색 알림표시줄이나 하단의
                             [수동설치]를 통해 Payplus Plug-in 설치가 가능합니다.
                             결제요청 버튼을 클릭하게 될 경우 Payplus Plug-in이 실행되며 Payplus Plug-in을 통해 결제요청 정보를 암호화하여
                             결제요청 페이지로 전송합니다.
                             소수 수정 시 ※ 필수, ※ 옵션표시가 포함된 문장은 가맹점의 상황에 맞게 적절히 수정 적용하시기 바랍니다. */
%>
...
%>
                    ...
                    <tr>
                        <th>주문 번호</th>
                        <td><input type="text" name="ordr_idxx" value="<%=vo.getOrderSeq()%>" maxlength="40"/></td>
                    </tr>
                    <!-- 상품명(good_name) -->
                    <tr>
                        <th>상품명</th>
                        <td><input type="text" name="good_name" value="<%=PgUtil.escapeCharForKcp(vo.getItemName())%>"/></td>
                    </tr>
                    <!-- 결제금액(good_mny) - ※ 필수 : 값 설정시 ,(콤마)를 제외한 숫자만 입력하여 주십시오. -->
                    <tr>
                        <th>결제 금액</th>
                        <td><input type="text" name="good_mny" value="<%=vo.getPayPrice()%>" maxlength="9"/>원(숫자만 입력)</td>
                    </tr>
                    <!-- 주문자명(buyr_name) -->
                    <tr>
                        <th>주문자명</th>
                        <td><input type="text" name="buyr_name" value="<%=PgUtil.escapeCharForKcp(vo.getMemberName())%>"/></td>
                    </tr>
                    <!-- 주문자 E-mail(buyr_mail) -->
                    <tr>
                        <th>E-mail</th>
                        <td><input type="text" name="buyr_mail" value="<%=PgUtil.escapeCharForKcp(vo.getBuyerEmail())%>" maxlength="30" /></td>
                    </tr>
                    <!-- 주문자 연락처1(buyr_tel1) -->
                    <tr>
                        <th>전화번호</th>
                        <td><input type="text" name="buyr_tel1" value="<%=vo.getReceiverTel()%>"/></td>
                    </tr>
                    <!-- 휴대폰번호(buyr_tel2) -->
                    <tr>
                        <th>휴대폰번호</th>
                        <td><input type="text" name="buyr_tel2" value="<%=vo.getReceiverCell()%>"/></td>
                    </tr>
                    </table>

                    <!-- 결제 요청/처음으로 이미지 -->
                    <%--<div class="btnset" id="display_pay_button" style="display:block">
                      <input name="" type="submit" value="결제요청" onclick="return jsf__pay(this.form);"/>
...
<% if(commFreeMny > 0) {%>
	...
	<input type="hidden" name="comm_tax_mny"   value="<%=commTaxSupplyMny%>"/>  <!-- 과세 공급가액 -->
	<input type="hidden" name="comm_vat_mny"   value="<%=commTaxVatMny%>"/>     <!-- 부가세	     -->
	<input type="hidden" name="comm_free_mny"  value="<%=commFreeMny%>"/>       <!-- 비과세 금액   -->
<%} %>
<%
