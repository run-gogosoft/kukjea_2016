...
<head>
	...
	<link href="${const.ASSETS_PATH}/front-assets/css/order/order.css" type="text/css" rel="stylesheet">
	<title>${title}</title>
</head>
<body>
				...
				<tr>
					<td>
						<c:if test="${item.img1 ne ''}">
							<img src="${const.IMG_DOMAIN}${const.UPLOAD_PATH}${fn:replace(item.img1, '/origin/', '/s206/')}" style="width:70px;border:1px solid #d7d7d7;" width="70px" alt="" />
						</c:if>
					</td>
...
<div class="cart-total-wrap">
	<div class="left">
		<div class="price-wrap-type2">
			<div>총 판매가 : <fmt:formatNumber value="${totalSellPrice}" pattern="#,###" />원</div>
			<div>할인금액: <span id="discountPriceText">0원</span></div>
		</div>
		<%-- 초기 디자인컨셉은 price-wrap-type1, 현재 적용은 price-wrap-type2 --%>
		<div class="price-wrap-type2" style="margin-top:8px">
			<div>배송비 합계: <fmt:formatNumber value="${totalDeliveryPrice}" pattern="#,###" />원</div>
		</div>
	</div>

	<div class="right">
		<div class="total-buy-price">총 구매금액: <span id="totalPriceText"><fmt:formatNumber value="${totalSellPrice + totalDeliveryPrice}" pattern="#,###" /></span> 원</div>
	</div>
</div>
...
<div class="sign-form form1">
	<div class="inner">
		<div class="title-back">
			<div class="title-wrap">
				<div class="title">01.주문자 정보</div>
			</div>
		</div>
		<table class="table sign-table">
			<tr>
				<td>주문자명<span>*</span></td>
				<td>
					<c:choose>
						<c:when test="${sessionScope.loginSeq eq null and sessionScope.loginName ne null}">
							<!-- 비회원 로그인일 경우 -->
							<input type="text" name="memberName" value="${sessionScope.loginName}" maxlength="15" alt="주문자명" class="form-control" readonly>
						</c:when>
						<c:otherwise>
							<input type="text" name="memberName" value="${memberVo.name}" maxlength="15" alt="주문자명" class="form-control">
						</c:otherwise>
					</c:choose>
			...
			<tr>
				...
				<td>
					<input type="text" name="memberCell1" value="${memberVo.cell1}" class="form-control" onblur="numberCheck(this);" alt="주문자 휴대폰 번호" maxlength="4">&nbsp;-&nbsp;
					<input type="text" name="memberCell2" value="${memberVo.cell2}" class="form-control" onblur="numberCheck(this);" alt="주문자 휴대폰 번호" maxlength="4">&nbsp;-&nbsp;
					<input type="text" name="memberCell3" value="${memberVo.cell3}" class="form-control" onblur="numberCheck(this);" alt="주문자 휴대폰 번호" maxlength="4">
				</td>
			</tr>
			<tr>
				<td>이메일<span>*</span></td>
				<td>
					<c:choose>
						<c:when test="${sessionScope.loginSeq eq null and sessionScope.loginEmail ne null}">
							<!-- 비회원 로그인일 경우 -->
							<input type="text" class="form-control" style="width:250px;" name="memberEmail" value="<smp:decrypt value="${sessionScope.loginEmail}"/>" maxlength="100" placeholder="example@example.com" alt="이메일" readonly/>
						</c:when>
						<c:otherwise>
							<input type="text" class="form-control" style="width:250px;" name="memberEmail" value="${memberVo.email}" maxlength="100" placeholder="example@example.com" alt="이메일"/>
						</c:otherwise>
					</c:choose>
				</td>
			</tr>
...
<div class="sign-form form2" style="margin-top:35px;">
	<div class="inner">
		<div class="title-back">
			<div class="title-wrap">
				<div class="title">02.배송지 정보</div>
			</div>
		</div>
		<table id="deliveryTable" class="table sign-table">
			<tr>
				<td>수신자명<span>*</span></td>
				<td>
					<input type="text" name="receiverName" maxlength="15" data-name="name" class="form-control" style="float:left;" alt="받으시는 분" />
					<c:if test="${sessionScope.loginSeq ne null}">
						<!-- 회원일 경우 -->
						<div class="order-radio-list">
							<div><input type="radio"  name="deliveryMethod" onclick="CHProcess.deliveryAddrMethod(this)" value="default" checked>&nbsp;기본 배송지</div>
							<div><input type="radio"  name="deliveryMethod" onclick="CHProcess.deliveryAddrMethod(this)" value="lately">&nbsp;최근 배송지</div>
							<div><input type="radio"  name="deliveryMethod" onclick="CHProcess.deliveryAddrMethod(this)" value="buyer">&nbsp;주문자 정보와 동일</div>
							<!-- <li><input type="radio"  name="deliveryMethod" >&nbsp;주문자정보와 동일</li> -->
							<div><input type="radio"  name="deliveryMethod" onclick="CHProcess.deliveryAddrMethod(this)" value="new">&nbsp;새로입력</div>
						</div>
					</c:if>
					<c:if test="${sessionScope.loginSeq eq null and sessionScope.loginEmail ne null}">
						<!-- 비회원 로그인일 경우 -->
						<div class="order-radio-list">
							<div style="width:200px"><input type="radio" name="deliveryMethod" onclick="CHProcess.deliveryAddrMethod(this)" value="notlogin">&nbsp;주문자 정보와 동일</div>
						</div>
					</c:if>
				</td>
			</tr>
			<tr>
				...
				<td>
					<input type="text" name="receiverTel1" data-name="tel1" alt="전화번호" onblur="numberCheck(this);" maxlength="4" class="form-control">&nbsp;-&nbsp;
					<input type="text" name="receiverTel2" data-name="tel2" alt="전화번호" onblur="numberCheck(this);" maxlength="4" class="form-control">&nbsp;-&nbsp;
					<input type="text" name="receiverTel3" data-name="tel3" alt="전화번호" onblur="numberCheck(this);" maxlength="4" class="form-control">
				</td>
			</tr>
			<tr>
				<td>휴대폰번호<span>*</span></td>
				<td>
					<input type="text" name="receiverCell1" data-name="cell1" alt="휴대폰 번호" onblur="numberCheck(this);" maxlength="3" class="form-control">&nbsp;-&nbsp;
					<input type="text" name="receiverCell2" data-name="cell2" alt="휴대폰 번호" onblur="numberCheck(this);" maxlength="4" class="form-control">&nbsp;-&nbsp;
					<input type="text" name="receiverCell3" data-name="cell3" alt="휴대폰 번호" onblur="numberCheck(this);" maxlength="4" class="form-control">
				</td>
			</tr>
			<tr>
				<td>이메일</td>
				<td>
					<input type="text" class="form-control" style="width:250px;" name="receiverEmail" value="" maxlength="100" placeholder="example@example.com" data-name="email" />
				</td>
			</tr>
			<tr>
				<td>주소<span>*</span></td>
				<td>
					<input type="text" class="form-control" id="postcode" name="postcode" data-name="postcode" maxlength="5" alt="우편번호" readonly />
					<button type="button" class="btn btn-default" onclick="CHPostCodeUtil.postWindow('open', '#deliveryTable');"><span>우편번호찾기</span></button>
					<br/>
					<input type="text" class="form-control addr" id="addr1" name="addr1" data-name="addr1" maxlength="100" alt="주소" readonly /><br/>
					<input type="text" class="form-control addr" id="addr2" name="addr2" data-name="addr2" maxlength="100" alt="상세주소" />
			...
			<tr>
				<td>배송 메세지</td>
				<td>
					<input type="text"  name="request" maxlength="500" class="form-control" style="width:400px;" placeholder="ex) 배송 전 연락 바랍니다. 부재 시 경비실에 맡겨 주세요."/>
				</td>
			</tr>
		</table>
	</div>
...
<c:if test="${sessionScope.loginSeq > 0}">
	<div class="cart-button-wrap">
		<div class="pull-left" style="margin-top:11px">
			<button type="button" class="btn cart-small-btn btn-all-select" onclick="CHDelivery.deliveryListShow()">
				<span>다른배송지로 변경하기</span>
			</button>
		</div>
...
<%-- 결제 정보 --%>
<div class="sign-form form3" style="margin-top:35px; height:auto">
	<div class="inner" style="height:auto;">
		<div class="title-back">
			<div class="title-wrap">
				<div class="title">03.결제 정보</div>
			</div>
		</div>
		<table class="table sign-table">
			<c:if test="${sessionScope.loginSeq ne null}">
								...
								<c:if test="${item.value ne 'POINT' and fn:contains(mallVo.payMethod, item.value)}">
									...
									<c:if test="${sessionScope.loginMemberTypeCode ne 'P' and item.value eq 'CASH'}">
										...
										<div style="display:block; float:none; margin:10px 0; width:740px;">
											<input type="radio" name="payMethod" value="${item.value}" onclick="checkPayMethod(this)" />&nbsp;${item.name}
											(
												입금은행: <strong class="text-info">신한은행</strong> /
												계좌번호: <strong class="text-info">100-031-304688</strong> /
												예금주: <strong class="text-info">(재)행복한웹앤미디어 김병두</strong>
											)
										</div>
									</c:if>
								</c:if>
							</c:forEach>
							<input type="hidden" name="accountInfo" value="신한은행 100-031-304688 (재)행복한웹앤미디어 김병두"/>
						</div>
					</td>
				</tr>
			</c:if>
...
<c:if test="${sessionScope.loginSeq ne null and sessionScope.loginMemberTypeCode ne 'C'}">
<div class="sign-form form3" style="margin-top:35px; height:auto; display:none;" id="tax_request">
	<div class="inner" style="height:auto;">
		<div class="title-back">
			<div class="title-wrap" style="width:400px">
				<div class="title" style="width:400px">04.세금계산서 요청서</div>
			</div>
		</div>
		<table class="table sign-table">
		<tr>
...
<div class="last-total-wrap">
	<div class="left">
		<div class="price-wrap">
			<div>총상품금액 : <fmt:formatNumber value="${totalSellPrice}" pattern="#,###" />원</div>
			<c:if test="${fn:contains(mallVo.payMethod, 'POINT')}"><div>총 할인금액 : <span id="totalDiscountPriceText">0</span> 원</div></c:if>
		</div>
	...
	<div class="right">
		<div class="total-buy-price">최종 구매금액: <span id="payPriceText" class="final-pay-amount"><fmt:formatNumber value="${totalSellPrice + totalDeliveryPrice}" pattern="#,###" /></span> 원</div>
	</div>
</div>

<div class="button-wrap">
	<button  type="submit" id="orderSubmitBtn" class="btn btn-buy"><span>주문하기</span></button>
</div>
<div class="clearfix"></div>
...
<%@ include file="/WEB-INF/jsp/shop/order/inc_mod_member_modal.jsp" %>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.order').find('img').attr("src", $('.order').find('img').attr("src").replace("_off", "_on"));
		var cartSeq="";
