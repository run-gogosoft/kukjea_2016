				...
				<c:if test="${item.noticeFlag eq 'Y'}">
					<tr>
						...
						<td class="tr-notice text-left">
							<a href="${item.content}" target="_blank"><strong>${item.title}</strong></a>
					...
					<tr>
						...
						<td>
							<div class="text-left title-div">
								<a href="#" onclick="showModal(this, '${item.title}', '${item.answer}')">${item.title}</a>
								<c:if test="${item.secretFlag eq 'Y'}"><i class="fa fa-lock" style="margin-left:10px;color:#555"></i></c:if>
							</div>
							<div class="sr-only" data-name="content">${item.content}</div>
						</td>
						<td>
							<c:choose>
								<c:when test="${item.userSeq eq null}">
...
<script id="youtubeTemplate" type="text/html">
        <div class="modal-content">
            ...
            <div class="modal-body">
            	<iframe width="100%" height="400" src="//www.youtube.com/embed/<%="${answer}"%>" frameborder="0" allowfullscreen></iframe>
				<br/>
				{{html content}}
            </div>
...
function showModal(obj, title, answer) {
	var content = $(obj).parents('td').find('[data-name=content]').text();
	$('#ModalBody').html( $('#youtubeTemplate').tmpl({title:title, answer:answer, content:content}) );
	$('#Modal').modal();
}
</script>
</body>
