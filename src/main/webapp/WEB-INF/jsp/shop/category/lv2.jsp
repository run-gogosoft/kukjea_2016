          ...
          <ul class="ch-3col lv1-3col" style="margin-bottom:60px;">
            <c:forEach var="item" items="${list}">
                <li style="margin-top:23px;">
                	<div class="img"><a href="/shop/detail/${item.seq}"><img src="${const.UPLOAD_PATH}${fn:replace(item.img1, 'origin', 's270')}" <c:if test="${item.img2 ne ''}">data-src="${const.UPLOAD_PATH}${fn:replace(item.img2, 'origin', 's270')}"</c:if>/></a></div>
                	<div class="info">
...
<script type="text/javascript">
      ...
      var submitCatagory = function (lvValue , obj, dataLv) {
            ...
            if($(obj).val() === '') {
                  if (lvValue === 1) {
                        //대분류 카테고리를 선택하지 않았을 경우 메인으로 간다.
                        urlDepth1 = "/shop/main";
                  } else {
                        //중,소,세 카테고리를 선택하지 않았을 경우 상위 카테고리 페이지로 간다.
            ...
            $('#searchForm').attr('action', urlDepth1 !== '' ? urlDepth1 : urlDepthEtc);
            $('#searchForm').submit();
      };
</script>
</body>
