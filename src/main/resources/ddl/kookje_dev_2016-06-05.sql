...
CREATE TABLE `sm_order_detail` (
  ...
  `mod_date` datetime DEFAULT NULL COMMENT '변경일',
  `reg_date` datetime NOT NULL COMMENT '등록일',
  `old_item_seq` varchar(10) DEFAULT NULL COMMENT '함께누리몰 기상품번호(item_seq,seller_seq 매칭용)',
  `old_order_id` varchar(10) DEFAULT NULL COMMENT '함께누리몰 기주분번호(order_seq 매칭용)',
  PRIMARY KEY (`seq`),
...
CREATE TABLE `sm_order_pay` (
  ...
  `reg_date` datetime NOT NULL COMMENT '등록일자',
  `old_order_id` varchar(10) DEFAULT NULL COMMENT '함께누리몰 기주문아이디(order_seq매칭용)',
  PRIMARY KEY (`seq`),
