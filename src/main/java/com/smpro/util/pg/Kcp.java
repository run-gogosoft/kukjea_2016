...
public class Kcp {
	...
	private static final String g_conf_home_dir = "C:\\dev\\workspace\\gsretail\\WebContent\\WEB-INF\\conf\\kcp";// BIN 절대경로 입력 (bin전까지)
	private static final String g_conf_key_dir = g_conf_home_dir + "\\bin\\pub.key"; // 공개키 파일 절대경로
	private static final String g_conf_log_dir = g_conf_home_dir + "\\log"; // LOG 디렉토리 절대경로 입력
	...
	public OrderPayVo doPay(HttpServletRequest request, int calcPayPrice) throws Exception {
		...
		if (StringUtil.isNum(request.getParameter("comm_free_mny"))) {
		...
		if (request.getParameter("tran_cd").length() > 0) {
			c_PayPlus.mf_do_tx(siteCd, siteKey, request.getParameter("tran_cd"), custIp,	ordrIdxx, g_conf_log_level);
		} else {
			c_PayPlus.m_res_cd = "9562";
			c_PayPlus.m_res_msg = "연동 오류|Payplus Plugin이 설치되지 않았거나 tran_cd값이 설정되지 않았습니다.";
		}
		tno = c_PayPlus.mf_get_res("tno") == null ? "" : c_PayPlus.mf_get_res("tno").trim();
		LOGGER.info("### KCP 결제 ");
		LOGGER.info("### tno : " + tno);
	...
	public OrderPayVo doCancel() {
		...
		c_PayPlus.mf_do_tx(siteCd, siteKey, "00200000", "", ordrIdxx, g_conf_log_level);
		return createOrderPayVo(c_PayPlus);
	}

	// 결제 취소
	public OrderPayVo doCancel(OrderPayVo vo, String taxCode) {
		if ("service".equals(Const.LOCATION)) {
			siteCd = vo.getPgId();
			siteKey = vo.getPgKey();
		}
		...
		if (vo.getPartCancelAmt() > 0) {
			...
			c_PayPlus.mf_set_us(mod_data_set_no, "rem_mny", String.valueOf(vo.getCurAmount())); // 취소가능잔액 
		...
		c_PayPlus.mf_do_tx(siteCd, siteKey, "00200000", "", String.valueOf(vo.getOrderSeq()), g_conf_log_level);

		return createOrderPayVo(vo, c_PayPlus);
	}

	// 결제 및 자동취소 처리 결과 맵핑
	private OrderPayVo createOrderPayVo(C_PP_CLI c_PayPlus) {
	...
	// 결제 취소 처리 결과 맵핑
	private OrderPayVo createOrderPayVo(OrderPayVo payVo, C_PP_CLI c_PayPlus) {
		OrderPayVo vo = new OrderPayVo();
