...
public class C_PP_CLI {
	...
	/* - 2006/12/12 - 로그관련 설정 - */
	/* -------------------------------------------------------------------- */
	private String m_c_strKeyPath = ""; // KEY PATH
	private String m_c_strLogPath = ""; // LOG PATH
	...
	public void mf_init(String parm_c_strHomeDir, String parm_c_strPAURL,
			String parm_c_strPAPorts, String parm_c_strKeyPath,
			String parm_c_strLogPath, int parm_nTxMode) {
		this.m_bSetEnv = true;
		...
		this.m_c_strKeyPath = parm_c_strKeyPath;
		this.m_c_strLogPath = parm_c_strLogPath;
	}
	...
	private String cf_do_tx_exe() {
		...
		try {
			...
			/*
			 * c_straCmd = new String[] { this.m_c_strHomeDir + "\\bin\\pp_cli",
			 * "\"" + "site_cd=" + this.m_c_strSite_CD + "," + "site_key=" +
			 * this.m_c_strSite_Key + "," + "tx_cd=" + this.m_c_strTx_CD + "," +
			 * "pa_url=" + this.m_c_strPAURL + "," + "pa_port=" +
			 * this.m_c_strPAPorts + "," + "ordr_idxx=" + this.m_c_strOrdr_IDxx
			 * + "," + "trace_no=" + this.m_c_strTraceNo + "," + "plan_data=" +
			 * "payx_data=" + this.m_c_strPayx_Data + "ordr_data=" +
			 * this.m_c_strOrdr_Data + "rcvr_data=" + this.m_c_strRcvr_Data +
			 * "escw_data=" + this.m_c_strEscw_Data + "modx_data=" +
			 * this.m_c_strModx_Data + "cust_ip=" + this.m_c_strCust_IP + "," +
			 * "log_level=" + this.m_c_strLogLevel + "," + "key_path=" +
			 * this.m_c_strKeyPath + "," + "log_path=" + this.m_c_strLogPath +
			 * "\"" };
			 */

			c_straCmd = new String[] {
					this.m_c_strHomeDir + "\\bin\\pp_cli_exe",
					"\"" + "site_cd=" + this.m_c_strSite_CD + "," + "site_key="
							+ this.m_c_strSite_Key + "," + "tx_cd="
							+ this.m_c_strTx_CD + "," + "pa_url="
							+ this.m_c_strPAURL + "," + "pa_port="
							+ this.m_c_strPAPorts + "," + "ordr_idxx="
							+ this.m_c_strOrdr_IDxx + "," + "enc_data="
							+ this.m_c_strEncData + "," + "enc_info="
							+ this.m_c_strEncInfo + "," + "trace_no="
							+ this.m_c_strTraceNo + "," + "cust_ip="
							+ this.m_c_strCust_IP + "," + "key_path="
							+ this.m_c_strKeyPath + "," + "log_path="
							+ this.m_c_strLogPath + "," + "log_level="
							+ this.m_c_strLogLevel + "," + "plan_data="
							+ this.m_c_strPayx_Data + this.m_c_strOrdr_Data
							+ this.m_c_strRcvr_Data + this.m_c_strEscw_Data
							+ this.m_c_strModx_Data + "\"" };
			...
			System.out.println("---------------------------------");
			System.out.println(c_straCmd[0]);
			System.out.println(c_straCmd[1]);
			System.out.println("---------------------------------");
		} catch (Exception e) {
			// bCont = false;

			c_strRT = "res_cd=S102" + String.valueOf((char) 0x1f)
					+ "res_msg=연동 모듈 실행 오류 : " + e;
		}
		/*
		 * finally { }
		 */
